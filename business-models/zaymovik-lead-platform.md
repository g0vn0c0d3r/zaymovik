...
graph LR
    ...
    C --> A["Оффер A: сайт Zaymovik"]
    C --> B["Оффер B: отдельный бренд"]
    C --> D["Оффер C: отдельный бренд"]

    A --> I["Единый приём заявок"]
    B --> I
    D --> I

    I --> V["Валидация, антифрод, дедупликация"]
    V --> R{"Балансировщик"}
    ...
    M1 --> S["Результат: отказ, одобрение или выдача"]
    M2 --> S
    M3 --> S
