...
```mermaid
flowchart LR
    T[Веб-мастера и финансовые витрины] --> C[CPA-сети]

    C -->|Оффер A| A[Сайт A<br/>Zaymovik]
    C -->|Оффер B| B[Сайт B<br/>отдельный бренд]
    C -->|Оффер C| D[Сайт C<br/>отдельный бренд]

    A --> I[Единый приём заявок]
    B --> I
    D --> I

    I --> V[Валидация<br/>антифрод<br/>дедупликация]
    V --> R{Балансировщик}
    ...
    M1 --> S[Статус<br/>отказ · одобрение · выдача]
    M2 --> S
    M3 --> S

    S --> E[Общая статистика и экономика]
    E -. корректировка правил .-> R
```
