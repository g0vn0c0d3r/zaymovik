...
> **Главное:** платформа Zaymovik не выдаёт деньги и не принимает кредитное решение. Она привлекает пользователей, собирает заявки и направляет каждую заявку одной подходящей МФО — собственной или партнёрской.
...
10. МФО возвращает статус: заявка принята, отказана, одобрена или займ выдан. Эти статусы используются для взаиморасчётов, анализа маршрутизации и настройки порога отсечения и весов.
11. Передача следующей МФО возможна только по заранее согласованному сценарию, например при технической недоступности первой МФО. Обычный кредитный отказ сам по себе не означает автоматическую рассылку заявки дальше.
...
**Масштабирование идёт с двух сторон:** новый сайт или оффер подключается к единому приёму заявок, а новая МФО — к пулу балансировщика. Поэтому входящий трафик и объём выдач можно увеличивать независимо друг от друга без перестройки ядра платформы.
