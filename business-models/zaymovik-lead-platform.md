...
> **Главное:** платформа Zaymovik не выдаёт деньги и не принимает кредитное решение. Она привлекает пользователей, собирает заявки и направляет каждую заявку одной подходящей МФО — собственной или партнёрской. После выдачи данные займа отображаются клиенту в едином ЛК Zaymovik.
...
- **МФО** — собственная или партнёрская микрофинансовая организация, которая оценивает клиента, принимает решение, заключает договор и выдаёт деньги.
- **Заявка** — данные, которые пользователь отправил через форму сайта, чтобы получить предложение займа.
- **ЛК Zaymovik** — единый личный кабинет, в котором клиент видит МФО, выдавшую займ, остаток долга, график и платежи независимо от сайта, с которого пришёл.
...
- **внутри** — единая база заявок, общие проверки, предварительный скоринг, один балансировщик, собственные МФО и возможность подключать партнёрские.
...
6. Прошедшая порог заявка вместе со скоринговым баллом поступает в балансировщик. Одновременно балансировщик получает данные о пуле доступных МФО: организация должна быть активна, её интеграция должна работать, лимит заявок не должен быть исчерпан, а **ликвидность** — возможность выдавать деньги сейчас — должна быть доступна.
...
8. Балансировщик выбирает одну МФО и направляет активную заявку только ей.
...
10. МФО возвращает статус заявки: принята, отказана, одобрена или займ выдан. После выдачи она также передаёт сведения о кредиторе, остатке долга, графике и платежах. Эти данные поступают в платформу через интеграцию с МФО.
11. Клиент видит данные выданного займа в едином ЛК Zaymovik и может войти в него с любого сайта платформы. Отдельный кабинет для каждого сайта не требуется; кредитором и источником данных остаётся выбранная МФО.
12. Передача следующей МФО возможна только по заранее согласованному сценарию, например при технической недоступности первой МФО. Обычный кредитный отказ сам по себе не означает автоматическую рассылку заявки дальше.
...
Zaymovik — собственная платформа с собственными сайтами, CPA-офферами и МФО.

**Масштабирование идёт с двух сторон:** новый сайт или оффер подключается к единому приёму заявок и готовому ЛК, а новая МФО — к пулу балансировщика. Поэтому входящий трафик и объём выдач можно увеличивать независимо друг от друга без перестройки ядра платформы.

Основа пула — собственные МФО. При необходимости к нему индивидуально подключаются выбранные партнёрские МФО с дополнительной ликвидностью. Балансировщик распределяет подходящие заявки между всеми активными МФО с учётом заданных весов, лимитов и текущей ликвидности.
