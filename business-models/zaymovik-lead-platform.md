...
По текущему опыту, чтобы один оффер получил около 1 млн кликов, ему нужен EPC порядка 150 ₽ и выше. Если тот же объём распределить между тремя отдельными офферами, каждому достаточно выкупить примерно треть миллиона кликов и конкурировать за меньшую часть трафика.

```text
1 домен:
1 оффер × 1 000 000 кликов → требуемый EPC 150+ ₽

3 домена:
Оффер A × ~333 000 кликов ─┐
Оффер B × ~333 000 кликов ─┼→ около 1 000 000 кликов суммарно
Оффер C × ~334 000 кликов ─┘

Требуемый EPC каждого оффера → около 90 ₽
```

Поэтому при прочих равных портфель из нескольких доменов может набрать тот же миллион кликов при более низком EPC каждого оффера. Экономию создаёт не сам факт наличия разных доменов, а разделение большого объёма между несколькими самостоятельными карточками и ротациями.
...
Визуальная версия этой же схемы:
